...
if ( $previous_post ) {
	// text above the link
	$previous_text = esc_html__( 'Previous Post', 'challenger' );
	// if there is a title use it, else call it "The Previous Post"
	$previous_title = get_the_title( $previous_post ) ? get_the_title( $previous_post ) : esc_html__( 'The Previous Post', 'challenger' );
	// get the post link
	$previous_link = get_permalink( $previous_post );
} // if there isn't a previous post
else {
	// text above the link
	$previous_text = esc_html__( 'No Older Posts', 'challenger' );
	// set the title to return to the blog
	$previous_title = esc_html__( 'Return to Blog', 'challenger' );
	// link to blog
...
if ( $next_post ) {
	// text above the link
	$next_text = esc_html__( 'Next Post', 'challenger' );
	// if there is a title use it, else call it "The next Post"
	$next_title = get_the_title( $next_post ) ? get_the_title( $next_post ) : esc_html__( 'The Next Post', 'challenger' );
	// get the post link
	$next_link = get_permalink( $next_post );
} // if there isn't a next post
else {
	// text above the link
	$next_text = esc_html__( 'No Newer Posts', 'challenger' );
	// set the title to return to the blog
	$next_title = esc_html__( 'Return to Blog', 'challenger' );
	// link to blog
	$next_link = home_url();
}
